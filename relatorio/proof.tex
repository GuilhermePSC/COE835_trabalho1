...
Inicialmente, filtra-se o sinal de saída $y_p(t)$ utilizando o modelo de referência, produzindo o sinal de saída filtrada $\zeta(t)$ \eqref{eq:filter}. Este sinal é utilizado para a construção da estimativa do erro \eqref{eq:est_error} e na própria lei de adaptação do parâmetro $\theta$, dada em \eqref{eq:adpt_law}.
...
\end{flalign}
%
onde $\tilde{\theta} = \theta - \theta^*$, $\theta^*$ é o ganho de realimentação ideal e o operador $\mathcal{L}^{-1}\{*\}$ denota a transformada de Laplace inversa.
