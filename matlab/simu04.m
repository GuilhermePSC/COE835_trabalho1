...
hold on
grid on
plot(t,ym,t,yp,'Linew',0.5);

%---------------------------------------- Continuous simulation -----
Ns = 5       %N�mero de simula��es
Dt = 10;     %Incremento no tempo de simula��o
Dg = 0;      %Incremento de gamma
Da = 1;      %Incremento no par�metro ap

for i=2:Ns
    Xfinal = X(size(X,1),:);        %Estado final da �ltima simula��o
    X0 = Xfinal;             %Estado inicial para a pr�xima simula��o

    t0 = tfinal;      
    tfinal = t0 + Dt;
    intervalo = [t0 tfinal];     %Intervalo da pr�xima simula��o
    
    gamma = gamma + Dg;          %Altera��o de par�metros
    ap = ap + Da;
    P = ss(1/(s-ap));
    thetas = -ap - am   %theta*
    ...
    %[time,X] = sim('MRAC_111',intervalo,opcoes); %Continua��o!
    [time,X] = sim('MRAC_111',intervalo,opcoes); %Continua��o!
...
subplot(211)
title('e_0')
legend('e_0','Location','NorthEast')
print -depsc2 ../relatorio/figs/fig04a.eps

figure(2)
subplot(211)
title('\theta, \theta*')
legend('\theta','\theta*')
print -depsc2 ../relatorio/figs/fig04b.eps

figure(3)
subplot(211)
title('y_m, y_p')
legend('y_m','y_p')
print -depsc2 ../relatorio/figs/fig04c.eps
...
title('\theta, \theta*')
grid on; 
legend('\theta','\theta*','East')
print -depsc2 ../relatorio/figs/fig04d.eps
...
grid on; 
title('e_0')
legend('e_0','Location','SouthEast')

subplot(222)
plot(T,Theta,T,Thetas,'r','Linew',0.5);
grid on; 
title('\theta, \theta*')
legend('\theta','\theta*','Location','SouthEast')

subplot(223)
plot(T,YP,T,R,T,YM,'Linew',0.5);
grid on; 
title('r, y_m, y_p')
legend('y_p','r','y_m','Location','SouthEast')
